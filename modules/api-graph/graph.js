...
swagger.ed.Graph = (function () {
	...
	var _private = {
		...
		'draw': function draw( api, opts ){
			...
			/*
			 * parse the Swagger endpoints and
			 * create nodes
			 */
			...
			try {
				...
				$('body').append("<div id='container-graph' class='swagger-graph'></div>");
				...
				_globals.network = new vis.Network($('#container-graph')[0], data, options);
		...
		/**
		 * Scans the graph and 
		 * tries to detect a node
		 * with the given Id and parent.
		 */
				...
				if (typeof info != 'undefined') {
					...
					if (typeof info.license != 'undefined') {
						...
						html += "<dl class='dl-horizontal'><dt>URL</dt><dd><a href='"
								+ info.license.url + "' target='_blank'>"
								+ info.license.url + "</a></dd></dl>";
					}
			...
			try {
				...
				for ( var key in path) {
					...
					html += "<li role='presentation' class='operation  "
							+ (count == 1 ? 'active ' : '') + key + "' ><a href='#"
							+ key + "' aria-controls='" + key
							+ "' role='pill' data-toggle='pill'>" + key.toUpperCase()
							+ "</a></li>"
				}
				...
				for ( var key in path) {
					...
					html += "<h4>"
							+ (typeof method.summary != 'undefined' ? method.summary
									: 'Description') + "</h4> <p>" + method.description
							+ "</p>";
					...
					if (typeof method.parameters != 'undefined') {
						...
						for ( var idx in method.parameters) {
							...
							// description
							html += "<td>" + (typeof param.description != 'undefined' ? param.description : '') + "</td>";

							// required?
							html += "<td><span class='glyphicon "
									+ (param.required ? "glyphicon-ok"
											: "glyphicon-remove")
					...
					if (typeof method.consumes != 'undefined') {
						...
						for ( var idx in method.consumes) {
							...
							html += "<span class='label label-info'>" + mime
									+ "</span>";
						}
					...
					if (typeof method.produces != 'undefined') {
						...
						for ( var idx in method.produces) {
							// get the next param
							var mime = method.produces[idx];
							html += "<span class='label label-primary'>" + mime
									+ "</span>";
						}
			...
			try {
				...
				if (typeof model.required != 'undefined') {
					...
					for ( var idx in model.required) {
						...
						html += "<span class='label label-warning'>" + property
								+ "</span>";
					}
		...
		'onSecurityDetails': function(node){
			...
			try {
				...
				if (policy.type === 'apiKey') {
					...
					html += "<dl class='dl-horizontal'><dt>Flow:</dt><dd>"
							+ policy.flow + "</dd></dl>";
					...
					if (typeof policy.scopes != 'undefined') {
						...
						for ( var scope in policy.scopes) {
							html += "<tr><td><span class='label label-info'>" + scope
									+ "</span></td><td>" + policy.scopes[scope]
									+ "</td></tr>";
						}
