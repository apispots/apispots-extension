...
chrome.pageAction.onClicked.addListener(function(tab) {
	...
	chrome.windows.create({url: chrome.extension.getURL("canvas.html"), type: "popup"}, function() {
	
		// send a message to the new window with the API definition
		var msg = null;
		
		if (globals.type === 'api')
			msg = { name: 'onVisualizeApi', api: globals.api };
		else if (globals.type === 'catalog')
			msg = { name: 'onVisualizeCatalog', catalog: globals.catalog };
...
chrome.runtime.onMessage.addListener(function(msg, sender, cb) {
	...
	case 'onSwaggerDefinition':
		...
		chrome.pageAction.setIcon({path: 'res/swagger-32.png', tabId: sender.tab.id});
	...
	case 'onApiCatalogDefinition':	
		...
		// enable the page action
		chrome.pageAction.setIcon({path: 'res/catalog-32.png', tabId: sender.tab.id});
		chrome.pageAction.show(sender.tab.id);
		break;
